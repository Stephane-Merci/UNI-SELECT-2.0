-- Force-fix for production DBs that still reject per-worker machinery checks.
-- This runs safely on every environment:
-- 1) drop any unique constraint exactly on (planId, postId)
-- 2) drop the legacy unique index name if present
-- 3) ensure unique (planId, postId, workerId) exists
